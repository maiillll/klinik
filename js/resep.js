$(document).ready(function() {
    let selectedPatient = null, selectedTindakan = null, selectedObat = null;
    let itemsTindakan = [], itemsObat = [];
    ...
    function renderTablesAndTotal() {
        ...
        itemsTindakan.forEach((item, i) => {
            const subtotal = item.harga * item.qty; total += subtotal;
    ...
    setupAutocomplete('cariPasien', 'patients', data => { selectedPatient = data; $('#pasienInfo').text(`ID: ${data.id}, Alamat: ${data.alamat}`); });
    setupAutocomplete('cariTindakan', 'actions', data => { selectedTindakan = data; $('#hargaTindakan').val(formatRupiah(data.harga)); });
    setupAutocomplete('cariObat', 'medicines', data => { selectedObat = data; $('#hargaObat').val(formatRupiah(data.harga)); });
    $('#addTindakan').on('click', () => { if (selectedTindakan && $('#qtyTindakan').val() > 0) { itemsTindakan.push({ ...selectedTindakan, qty: Number($('#qtyTindakan').val()) }); renderTablesAndTotal(); $('#cariTindakan, #hargaTindakan').val(''); $('#qtyTindakan').val(1); selectedTindakan = null; } else { alert('Pilih tindakan & pastikan Qty > 0.'); } });
    $('#addObat').on('click', () => { if (selectedObat && $('#qtyObat').val() > 0) { itemsObat.push({ ...selectedObat, qty: Number($('#qtyObat').val()) }); renderTablesAndTotal(); $('#cariObat, #hargaObat').val(''); $('#qtyObat').val(1); selectedObat = null; } else { alert('Pilih obat & pastikan Qty > 0.'); } });
    $('#tabelTindakan').on('click', '.remove-tindakan', function() { itemsTindakan.splice($(this).data('index'), 1); renderTablesAndTotal(); });
    $('#tabelObat').on('click', '.remove-obat', function() { itemsObat.splice($(this).data('index'), 1); renderTablesAndTotal(); });
    $('#formResep').on('submit', function(e) {
        ...
        const tagihanData = {
            ...
            itemsTindakan, itemsObat, total: totalBiaya, status: 'PENDING',
            dibuatPada: firebase.firestore.FieldValue.serverTimestamp()
        };
        db.collection('bills').add(tagihanData).then(docRef => {
            alert(`Tagihan berhasil dibuat!\nID: ${docRef.id}\n\nSIMULASI: Proses selanjutnya adalah memanggil backend untuk pembayaran Midtrans. Karena ini hanya frontend, kita akan dialihkan ke halaman riwayat.`);
            window.location.href = 'pembayaran.html';
        }).catch(err => { console.error(err); alert('Gagal membuat tagihan.'); });
    });
});
