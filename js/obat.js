$(document).ready(function() {
    ...
    db.collection('medicines').orderBy('nama').onSnapshot(snapshot => {
        ...
        snapshot.forEach((doc, index) => {
            ...
            table.row.add([
                index + 1, medicine.nama, medicine.satuan, formatRupiah(medicine.harga),
                `<button class="btn btn-danger btn-sm" onclick="deleteData('medicines', '${doc.id}')"><i class="fas fa-trash"></i></button>`
            ]).draw(false);
        });
    });
    $('#formData').on('submit', function(e) {
        ...
        db.collection('medicines').add({
            nama: $('#nama').val(), satuan: $('#satuan').val(), harga: Number($('#harga').val())
